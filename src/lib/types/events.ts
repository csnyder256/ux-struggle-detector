...
/**
 * Longest `route` the ingest schema keeps. The SDK clamps to it and the
 * server truncates anything longer: a batch is validated as a whole, so
 * rejecting one oversized route (a hash-routed app with a long path in its
 * fragment) would drop every event in the flush.
 */
